...
<head>
    ...
    <link href="<?= APP_URL ?>/public/css/bootstrap.min.css" rel="stylesheet">
</head>
<body>
    ...
    <table class="table table-bordered">
        ...
        <tbody>
            <?php foreach ($data['orderItems'] as $item): ?>
                <tr>
                    <td><img src="<?= APP_URL ?>/public/images/<?= htmlspecialchars($item['hinhanh']) ?>" style="max-width:60px;"></td>
                    <td><?= htmlspecialchars($item['tensp']) ?></td>
                    <td><?= htmlspecialchars($item['quantity']) ?></td>
                    <td><?= number_format($item['giaxuat'],0,',','.') ?>₫</td>
                    <td><?= number_format($item['thanhtien'] ?? ($item['giaxuat'] * $item['quantity']),0,',','.') ?>₫</td>
                </tr>
            <?php endforeach; ?>
        </tbody>
    </table>
